<%@ page language="java" import="java.util.*" pageEncoding="utf-8"%>
<%
String path = request.getContextPath();
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
%>
<%@page import="java.io.PrintWriter"%>
<%@page import="com.wea.servlet.SelectServlet"%>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
<html>
  <head>
    <base href="<%=basePath%>">
    
    <title>My JSP 'showImg.jsp' starting page</title>
    
	<meta http-equiv="pragma" content="no-cache">
	<meta http-equiv="cache-control" content="no-cache">
	<meta http-equiv="expires" content="0">    
	<meta http-equiv="keywords" content="keyword1,keyword2,keyword3">
	<meta http-equiv="description" content="This is my page">
	<!--
	<link rel="stylesheet" type="text/css" href="styles.css">
	-->

  </head>
  <table width="800" border="0" cellpadding="0" cellspacing="0" align="top">
    <!--DWLayoutTable-->
    <tr>
      <td width="800" height="600">&nbsp;
      <%
  
  if(request.getAttribute("isExit")!=null){
boolean isExit = Boolean.parseBoolean(request.getAttribute("isExit").toString());
	if(isExit){
	String filename = SelectServlet.getImage(new PrintWriter(out), session,request);
	String imageURL = request.getContextPath()+"/servlet/DisplayChart?filename="+filename;
	
	 %>
	<img>
		<img alt="无法查看图片 " align="middle" src="<%=imageURL%>" usemap="<%=filename %>"/>
<%
}
}
 %>
      </td>
    </tr>
  </table>
  
  
</html>
